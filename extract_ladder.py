...
# -*- coding: utf-8 -*-
# =====================================================================
#  STAGE 2d -- EXTRACTION SCALE LADDER (bounded scoring, bigger LMs)
#
#  Tests the decomposition prediction: extraction recall rises with
#  scale; composite = recall x 1.00 (bank is exact).
#
#    python extract_ladder.py --quick                 (SmolLM2-360M-Instruct)
#    python extract_ladder.py --quick --base          (base variant)
#    python extract_ladder.py --quick --model HuggingFaceTB/SmolLM2-1.7B-Instruct
#
#  Run 360M-Instruct, 360M-base, then 1.7B if RAM allows (~7GB fp32).
#  Each run appends to smollm_results/ladder.json -> the scaling curve.
# =====================================================================
...
# -------------------------- scoring -------------------------------- #
@torch.no_grad()
def score_pairs(model, tok, story, names, bs=8):
    ...
    for i0 in range(0, len(cands), bs):
        ...
        for i, (a, b) in enumerate(chunk):
            ...
            start = len(seqs[i]) - Lc
            tgt = seqs[i][start:]
            lp = logp[i, start - 1:start - 1 + Lc, :].gather(
                1, tgt.unsqueeze(1)).sum().item()
            out.append((lp / Lc, a, b))
    return out
...
def main():
    ...
    args = ap.parse_args()
    if args.base:
        args.model = args.model.replace("-Instruct", "")
    if args.quick:
        args.per_l = 2
    ...
    print("=" * 68)
    print(f" EXTRACTION SCALE LADDER -- {args.model}")
    print("=" * 68)
    self_test()
    from transformers import AutoModelForCausalLM, AutoTokenizer
    print(f"[lm] loading {args.model} (fp32 on CPU; 1.7B needs ~7GB RAM)...")
    t0 = time.time()
    ...
    print(f"[lm] loaded {nparams:.0f}M params in {time.time()-t0:.0f}s")

    lengths = [2, 4, 6, 8] if args.quick else list(range(2, 9))
    rows = []
    t0 = time.time()
    for L in lengths:
        for i in range(args.per_l):
            ...
            gold = rel_name(LL)
            rows.append(dict(L=LL, rec=rec, comp=(ans == gold)))
            if L == lengths[0] and i == 0:
                print(f"    [top-{k} edges sample] {edges[:5]}")
            print(f"  L={LL} {s}->{t}: extR={rec:.2f} composite={ans} "
                  f"({'OK' if ans == gold else 'FAIL'})", flush=True)
    print("\n" + "=" * 44)
    print(f"  {args.model}  ({nparams:.0f}M)")
    print("  L    extR   composite")
    for L in lengths:
        rs = [r for r in rows if r["L"] == L]
        print(f"  {L:<4} {sum(r['rec'] for r in rs)/len(rs):.2f}   "
              f"{100.0*sum(r['comp'] for r in rs)/len(rs):.1f}%")
    R = sum(r["rec"] for r in rows) / len(rows)
    C = 100.0 * sum(r["comp"] for r in rows) / len(rows)
    print(f"  ALL  {R:.2f}   {C:.1f}%")
    print(f"\n[SCALED EXTRACTION] recall={R:.2f}, composite={C:.1f}% "
          f"(135M scoring was 0.31/16.7%)")
    ladder_path = _os.path.join(args.outdir, "ladder.json")
    entry = dict(model=args.model, params_M=round(nparams, 1),
                 recall=round(R, 3), composite=round(C, 1))
    lad = []
    if _os.path.exists(ladder_path):
        with open(ladder_path) as f:
            lad = json.load(f)
    lad = [x for x in lad if x["model"] != args.model] + [entry]
    with open(ladder_path, "w") as f:
        json.dump(lad, f, indent=2)
    print(f"[appended] {ladder_path} -- run other sizes, then compare.")
